...
1. **ドキュメントの配置先:**
   - 各スタイルの解説・まとめドキュメントは、プロジェクトルート直下の [docs/knowledge/](file:///home/minamo/repository/exercises_in_style/docs/knowledge/) ディレクトリ配下に作成・格納してください。
   - ファイル名は `style-XX-style-name.md` のようにナンバリングとスタイル名を含めてください。

2. **テンプレートの利用:**
   - 新しいスタイルをドキュメント化する際は、必ず [.agents/templates/okf_programming_style.md](file:///home/minamo/repository/exercises_in_style/.agents/templates/okf_programming_style.md) のテンプレートを使用してください。

3. **OKF Frontmatter の必須項目:**
   - `type`: `programming-style` としてください。
   - `title`: スタイルの正式名称（例: "01. Good Old Times"）
   - `description`: そのスタイルの制約や特徴の端的な説明。
   - `resource`: 自身で実装したソースコードへの相対リンク（例: `file:///home/minamo/repository/exercises_in_style/01-good-old-times/main.py` など）。
